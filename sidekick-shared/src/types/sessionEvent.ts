/**
 * @fileoverview Canonical session event types shared across VS Code extension and CLI.
 *
 * These types define the rich event model used for parsing and monitoring
 * Claude Code, OpenCode, and Codex CLI sessions. SessionEvent is the canonical
 * type; ClaudeSessionEvent is a backward-compatible alias.
 *
 * @module types/sessionEvent
 */
...
export interface SessionMessage {
  ...
  /**
   * `'correction'` marks usage that tops up a call already counted (a later
   * split line of the same response reported more tokens). Add it to token and
   * cost totals, but do not count another call or context sample. Absent means
   * a regular call.
   */
...
/**
 * Canonical session event from JSONL/DB log files.
 *
...
/**
 * Backward-compatible alias for SessionEvent.
 * @deprecated Use SessionEvent instead.
 */
export type ClaudeSessionEvent = SessionEvent;
...
export interface TokenUsage {
  ...
  /**
   * `'correction'` tops up a call already counted (a later split line of the
   * same response reported more tokens): add it to totals, but do not count a
   * call or treat it as a context-size sample.
   */
  usageKind?: 'call' | 'correction';
}
...
export interface TimelineEvent {
  ...
  metadata?: {
    ...
    toolUseId?: string;
    isError?: boolean;
    /** Tokens billed for this response, cache included (see `summarizeTokens`). */
    tokenCount?: number;
    fullText?: string;
...
export interface SubagentStats {
  ...
  /** Tokens read from the prompt cache by this subagent */
  cacheReadTokens?: number;

  /** Tokens written to the prompt cache by this subagent */
  cacheWriteTokens?: number;

  /** Reasoning tokens reported by this subagent (display only) */
  reasoningTokens?: number;

  /** Every billed token, cache included — the same vocabulary as `summarizeTokens().total` */
  totalTokens?: number;
