...
describe('formatSessionText', () => {
  ...
  it('includes model stats when present', () => {
    const result = formatSessionText(
      mockMetrics({
        modelStats: [
          {
        ...
        subagents: [
          {
            ...
            spawnTime: '2025-06-15T10:01:00Z',
            status: 'completed',
            durationMs: 12000,
          },
        ],
  ...
  it('includes compaction events with timestamps', () => {
    const result = formatSessionMarkdown(
      mockMetrics({
        ...
        compactionEvents: [
          {
            timestamp: new Date('2025-06-15T10:03:00Z'),
...
describe('helper functions', () => {
  ...
  describe('formatDuration', () => {
    ...
    it('formats minutes and seconds', () =>
      expect(formatDuration('2025-01-01T00:00:00Z', '2025-01-01T00:05:30Z')).toBe('5m 30s'));
    it('formats hours', () =>
      expect(formatDuration('2025-01-01T00:00:00Z', '2025-01-01T01:30:15Z')).toBe('1h 30m 15s'));
    it('returns N/A for null start', () =>
      expect(formatDuration(null, '2025-01-01T00:00:00Z')).toBe('N/A'));
    it('returns N/A for null end', () =>
      expect(formatDuration('2025-01-01T00:00:00Z', null)).toBe('N/A'));
    it('returns N/A for negative duration', () =>
      expect(formatDuration('2025-01-01T01:00:00Z', '2025-01-01T00:00:00Z')).toBe('N/A'));
  });
});
