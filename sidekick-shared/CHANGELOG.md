...
- `collectSessionPromptHistory()` (`sidekick-shared/node` and the package root, not the browser entry) returns every human prompt of each matching Claude Code or Codex session, grouped by session.
  ...
  - **Bounds.** `maxSessions`, `maxSessionBytes` (default 256 MiB), `maxTotalBytes` (default 256 MiB), `maxRecordBytes`, `deadlineMs`, and `signal`. A session that `maxSessions`, `maxTotalBytes`, `deadlineMs`, or `signal` interrupts, or that changes while it is read, is listed in `unread`, never returned partially; a session over `maxSessionBytes` is returned with `truncated: true`.
...
- The Codex provider's six recursive walkers (session discovery, listing, id lookup, project folders, subagent scan) and the quota resolver's separate copy all use the capped walker; `getAllProjectFolders()` reads `session_meta` only for rollouts newer than the SQLite index's mtime when the index answered, and every discovery path uses the per-path cwd cache (now bounded to 5 000 entries) instead of re-reading the first line of every rollout; `scanSubagents()` stops after the 50 newest-dated rollouts instead of enumerating the whole history; the local Codex quota scan walks at most `maxSessionFiles` rollouts and no longer stats every path a second time to sort it; `ClaudeCodeProvider.listAllSessionFiles()` no longer stats every session twice
