...
- Bundles `sidekick-shared` 0.27.2 (streamed, resumable prompt history); no CLI-visible changes.

## [0.27.1] - 2026-09-22

### Changed

- Bundles `sidekick-shared` 0.27.1 (prompt-history late answer metadata); no CLI-visible changes.
...
- `sidekick dashboard` newer-session detection reconciles each watched-file event with one stat, and catch-up polls cost one stat per unchanged directory instead of walking every session file.
