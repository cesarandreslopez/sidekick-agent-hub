...
export function buildNarrativePrompt(
  ...
  const taskLines =
    metrics.tasks.length > 0
      ? metrics.tasks
          .map((tk) => `  - "${tk.subject}" (${tk.status}, ${tk.toolCallCount} tool calls)`)
          .join('\n')
      : '  (no tasks tracked)';
  ...
  const modelLines =
    metrics.modelStats.length > 0
      ? metrics.modelStats
          .map(
            (m) =>
              `  - ${m.model}: ${formatCost(m.cost)} (${m.calls} calls, ${fmtNum(m.tokens)} tokens)`,
          )
          .join('\n')
      : '  (no model data)';
  ...
  const topTools = metrics.toolStats
    .slice(0, 5)
    .map((ts) => `${ts.name}(${ts.calls})`)
    .join(', ');
