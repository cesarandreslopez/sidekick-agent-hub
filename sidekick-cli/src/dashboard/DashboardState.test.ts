...
describe('DashboardState', () => {
  ...
  describe('compaction detection', () => {
    it('counts explicit summary events', () => {
      ...
      state.processEvent(
        makeEvent({ type: 'assistant', summary: 'Hello', timestamp: '2025-01-01T00:00:02Z' }),
      );
      state.processEvent(
        makeEvent({
          type: 'summary',
    ...
    it('records one compaction for a summary event followed by the context drop', () => {
      const state = new DashboardState();
      ...
      // Summary event fires (no tokens) — aggregator records a pending heuristic compaction
      state.processEvent(
        makeEvent({
          type: 'summary',
          summary: 'Context compacted',
      ...
      // Next assistant event supplies the real post-compaction size, completing
      // the pending event instead of double-counting via drop detection.
      state.processEvent(
        makeEvent({
          type: 'assistant',
          model: 'claude-sonnet-4-20250514',
    ...
    it('injects compaction into timeline', () => {
      ...
      const m = state.getMetrics();
      const compactionEntry = m.timeline.find(
        (e) => e.type === 'summary' && e.summary.includes('compacted'),
      );
      ...
      state.processEvent(
        makeEvent({
          ...
          raw: {
            id: 'toolu_xyz',
            name: 'TaskUpdate',
            input: { taskId: '5', status: 'in_progress', subject: 'Imported task' },
          },
        }),
      );
      const m = state.getMetrics();
      ...
      state.processEvent(
        makeEvent({
          ...
          raw: {
            ...
            content: '{"taskId": "42", "status": "pending"}',
          },
        }),
      );
      const m = state.getMetrics();
      ...
      state.processEvent(
        makeEvent({
          ...
          raw: {
            id: 'a1',
            name: 'Task',
            input: { description: 'Agent A', subagent_type: 'Explore' },
          },
        }),
      );
