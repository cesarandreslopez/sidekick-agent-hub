...
- Bundles `sidekick-shared` 0.27.5 (new `collectSessionPromptHistory()`, which returns human prompts grouped by session); no extension-visible changes.

## [0.27.4] - 2026-09-23

### Changed

- Bundles `sidekick-shared` 0.27.4 (new `sidekick-shared/node` exports and an optional abort signal for `collectUsageEvents()`); no extension-visible changes.
...
- Bundles `sidekick-shared` 0.27.2 (streamed, resumable prompt history); no extension-visible changes.

## [0.27.1] - 2026-09-22

### Changed

- Bundles `sidekick-shared` 0.27.1 (prompt-history late answer metadata); no extension-visible changes.
...
- Terminal inference failures distinguish refresh-token rejection from expired conversations and recognize expired-login and reauthentication messages with guidance appropriate to the known credential kind.
...
- History tab depth: **Today** shows hourly buckets instead of one bar; a **Series** select stacks the chart by model or by tool (tool series plot calls); a **Project** select filters to one workspace over the last 500 session records; a dashed previous-period line (yesterday, the prior seven days, or the previous month) overlays the chart and the summary tiles show the change versus it. `requestHistoricalData` accepts `series` and `project`, and `HistoricalSummary` carries `series`, `seriesKeys`, `projects`, `project`, `previousPeriod`, and per-point `breakdown`
- A **Health** tab in the dashboard: the shared doctor report (status banner, checks with repair hints, deprecated settings), diagnostics the session providers emit when probed for the workspace, and a failing-tools table over the last 7 and 30 days with a trend arrow; `Sidekick: Doctor` keeps its Output-channel report and now also focuses the tab
...
- The extension advances in lockstep with `sidekick-shared` 0.25.0 and retains its existing extension commands and settings
...
- The context-attribution palette was a One Dark set declared in three places, one of which was already dead. Mind-map node colors were hardcoded too, and set with an SVG presentation attribute that cannot resolve theme variables — they now use a CSS `fill` property, which can. `url` and the three plan-step complexity tints are mixed away from the plain chart colors so they stay distinct from `plan`, `command`, `TODO`, and `tool`, and the palette's distinctness test now compares resolved expressions — a `var()` fallback that never applies had made two identical colors look different
- The mind-map legend listed ten of the twelve node types. It is generated from the same map the graph reads, and each row carries its node type in `data-node-type`, so hover-to-highlight and click-to-lock read the type off the row rather than mapping DOM position through a second, separately maintained list
...
- Nine commands are registered only inside the monitoring branch but are contributed to the Command Palette unconditionally — `importHistoricalData`, `openConversation`, `searchSessions`, `openToolInspector`, `generateSessionSummary`, `addKnowledgeNote`, `injectKnowledgeNotes`, `setupHandoff`, and `openExternalHandoff`. With monitoring off, invoking any of them — or Add Knowledge Note in the editor context menu, or the walkthrough's command link — raised `command not found`. The disabled branch now registers a fallback for each that names the setting to change. This predates the view gating by many releases
- `sidekick.knowledgeNotes` is revealed by `sidekick.monitoringActive`, but `KnowledgeNoteService` requires a workspace folder, so a folderless window contributed the view with no data provider and it sat on a loading spinner. It now registers an empty provider instead
...
- The pricing-catalog cache resolved a hardcoded `~/.config/sidekick`, the only one of the extension's four config-root resolutions without the `%APPDATA%` branch on Windows. Windows users got a cache at `%USERPROFILE%\.config\sidekick` while the CLI used `%APPDATA%\sidekick`, so each refetched the LiteLLM catalog on its own 24-hour TTL. It now falls through to the shared config directory
...
- Session reads inherit the shared transcript provenance surface: entrypoint, meta/sidechain flags, original roles, cwd, and git branch flow through Codex and Claude Code session parsing
...
- **Codex multi-home session discovery**: `ProviderDetector` now checks all candidate Codex home directories for activity, fixing missed session detection when the active managed profile has no sessions but the system `~/.codex/` does
...
- **OpenCode runtime notices**: The extension now shows an OpenCode-only actionable notice when `opencode.db` exists but `sqlite3` is missing, blocked, or otherwise unusable in the VS Code runtime
...
- **Toast Notifications**: Dismissable toast notifications in the dashboard with `aria-live` for screen readers — copying a CLAUDE.md suggestion now shows "Copied to clipboard" feedback
- **Reduced Motion Support**: `prefers-reduced-motion` media queries across all webviews — animations and transitions disabled when OS-level setting is enabled
- **Keyboard Navigation for Explain Panel**: Complexity level selector is now a segmented control with `role="tablist"` and roving tabindex — Arrow, Home, and End keys navigate between levels
- **Improved Focus Indicators**: Focus-visible outlines upgraded to 2px with positive offset across all webviews
- **ARIA Landmarks**: Explain and Error panels use semantic `<main>`, `role="region"`, `aria-live`, `role="status"`, `role="alert"`, and `aria-label` attributes
- **Theme-Aware Gauge Colors**: Context usage gauge reads colors from VS Code theme CSS variables instead of hardcoded RGB values
- **Redesigned Explain Panel**: Complexity selector replaced with a unified segmented control (bordered pill-style bar) with smooth hover/active transitions
- **Redesigned Error Panel**: Explanation sections now have color-coded left borders (red/orange/green), section icons, padded card-like backgrounds, and staggered slide-in entrance animations
- **Three-Dot Pulse Loader**: Replaced spinning circle loader with a three-dot pulse animation in Explain and Error panels
- **Improved Empty States**: All 7 sidebar panels have rewritten empty-state copy with icons, titles, and clearer instructions
- **Card Entrance Animations**: Fade-in animations with stagger delay in Task Board, Plan Board, and Project Timeline
- **Custom Scrollbar and Selection Styling**: Themed 6px scrollbars and VS Code-matching text selection colors across all webviews
- **Apply Fix Button States**: Animated "Applying..." state, green "Applied" confirmation, hover lift, and active press effects
- **Narrow Viewport Support**: Responsive styles for sidebar panels narrower than 260px

### Changed

- **Pulse Animation Throttling**: Value pulse animations throttled to once per 800ms per element, reducing visual noise during rapid updates
- **Inline Styles Replaced**: Context health score and truncation warnings now use CSS classes instead of inline styles

### Fixed

- **Codex SQL Parameter Binding**: Fixed `string.replace('?', ...)` only replacing the first placeholder — now correctly maps each `?` to its corresponding parameter
- **Codex Double Filesystem Stat**: Collapsed redundant `existsSync` + `statSync` in `CodexDatabase.isAvailable()` to a single `statSync` in try/catch
...
- **URL Protocol Validation**: `openExternal` handler validates `^https?://` before opening URLs
...
- **Session Dump Command**: `Sidekick: Dump Session` available from Command Palette, status bar menu, and Session Analytics toolbar — exports session data in text, markdown, or JSON
- **Plans Panel Re-enabled**: Plans sidebar panel restored with improved markdown parsing and plan file discovery from `~/.claude/plans/`
- **Session Introspection Views**: Enhanced Tool Inspector, Conversation View, Subagent Tree, and Temp Files views with introspection data
- **Enhanced Status Bar**: Monitor status bar now shows richer session summary information

### Fixed

- **Destructive command false positives**: Redirects to `/dev/null`, `/dev/stdout`, `/dev/stderr` no longer trigger the destructive command alert
- **Old snapshot format migration**: Restoring pre-0.12.3 session snapshots no longer shows empty timeline entries

## [0.12.5] - 2026-02-24

_No VS Code extension changes in this release._
...
- **Plan Analytics — Mind Map**: Enriched plan visualization
  ...
  - Enriched tooltips showing complexity, duration, tokens, tool calls, and errors
  - Failed steps shown with red stroke; skipped steps with dashed faded stroke
...
- **CLI version check**: When opening the CLI dashboard, the extension now checks if the installed CLI version is outdated compared to the extension version and prompts with an "Update Now" button (once per session)
...
- **"Open CLI Dashboard" Command**: New command `Sidekick: Open CLI Dashboard` launches the Sidekick TUI dashboard in a VS Code terminal
