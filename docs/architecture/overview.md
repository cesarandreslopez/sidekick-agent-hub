...
| `usage-cache/`                       | Per-session usage events extracted from session logs (Claude Code subagent transcripts included), keyed by size and mtime; feeds `sidekick blocks`, `sidekick daily`/`weekly`/`monthly`/`sessions`, and the dashboards' billing-block card (cache version 2 as of 0.27.3, rebuilt automatically; pruned least-recently-used) |
| `state.json`                         | Public, versioned (`schemaVersion: 1`) snapshot for external tools: account, quota windows with freshness, context usage (plus optional session `totalTokens` from the dashboards), session cost, active billing block; written by the status line and both dashboards only when changed                                     |
