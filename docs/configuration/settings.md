...
| `sidekick.accounts.keepAlive`           | `false` | Refresh saved-but-inactive accounts through the official `claude` / `codex` CLIs in their isolated profiles after activation and every six hours, so they do not expire while unused. |

When the threshold is greater than `0`, the extension host watches multi-provider quota and switches to another saved account for the active provider once utilization crosses the threshold. Account sign-in and switching are available from the **Accounts** view, the status bar badge, and the Command Palette (**Sidekick: Add Account…**, **Sidekick: Switch Account…**, `Ctrl+K Ctrl+Shift+A`). See the [Account Switcher](../features/account-switcher.md) guide and [Account Management](../providers/account-management.md).
...
| `sidekick.notifications.tokenThreshold`                | `5000000`  | Total-token (incl. cache) threshold; warns once per session, then at most every 30 min (0 = disabled) |
