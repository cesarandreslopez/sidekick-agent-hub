...
- **VS Code:** mind-map node colors are set with a CSS `fill` property rather than an SVG presentation attribute, which is what lets them resolve theme variables at all. The legend is generated from the same map the graph reads, fixing an existing drift where it listed ten of the twelve node types, and each row carries its own node type so hover-to-highlight and click-to-lock cannot fall out of step with the rows
...
- **VS Code:** with monitoring disabled, nine commands stayed in the Command Palette (and Add Knowledge Note in the editor context menu) while their handlers were registered only in the monitoring branch, so invoking one raised a raw `command 'sidekick.x' not found`. Each now explains that monitoring is off. Long-standing, not new in this release
- **VS Code:** in a window with no folder open, the Knowledge Notes view was revealed but its provider needs a workspace folder, leaving the view on a spinner. It now renders empty
...
- **VS Code:** the pricing-catalog cache resolved a hardcoded `~/.config/sidekick` — the one config root in the extension missing the Windows branch every other store uses. On Windows the extension and the CLI therefore kept separate caches and each refetched on its own 24-hour TTL. It now resolves through the shared config directory, which also means `SIDEKICK_CONFIG_DIR` moves it with everything else
...
- **CLI:** `sidekick extract -i --json` silently ignored `--json` and ran the interactive picker; the combination is now rejected with exit 1, and `extract -i` falls back to plain output when stdin or stdout is not a TTY
...
- Canonical transcripts in `sidekick-shared` retain provider provenance — entrypoint, meta/sidechain flags, original message roles, cwd, and git branch — so consumers can filter human prompts without reparsing JSONL
...
- Browser-safe provider usage normalization, non-double-counting pricing with provenance, deterministic token estimation, and canonical transcript projection in `sidekick-shared`
- Node recent-session/transcript read helpers and a resilient observed-session collector with provider/session isolation, bounded backoff, fingerprint bypass, duplicate suppression, and recovery diagnostics

### Changed

- Session events now use a discriminated contract: user/assistant rows require messages while valid summary/system bookkeeping rows may omit them
- Shared provider parsers and `EventAggregator` use normalized cache/reasoning semantics; VS Code diff budgeting uses the shared token estimator
- Provider-reported costs — including zero — are authoritative, so sessions that report $0 display $0 instead of a catalog estimate; observed-session `costUsd` is nullable when unpriced

### Deprecated

- Ambiguous legacy cost APIs remain compatible but are superseded by `calculateNormalizedUsageCost()`
...
### Changed (sidekick-cli, sidekick-vscode)
